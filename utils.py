    ...
    try:
        if os.path.exists(LAST_CHECK_FILE):
            with open(LAST_CHECK_FILE, "r") as f:
                ...
                logger.debug(
                    f"Read last check time: {timestamp} ({datetime.fromtimestamp(timestamp)})"
                )
                return timestamp
        else:
...
def save_last_check_time(timestamp):
    ...
    try:
        ...
        logger.debug(
            f"Saved last check time: {timestamp} ({datetime.fromtimestamp(timestamp)})"
        )
    except Exception as e:
...
def format_call_details(call):
    ...
    caller = (
        str(call.get("caller_id_number", "Unknown"))
        .replace("_", "\\_")
        .replace("*", "\\*")
    )
    destination = (
        str(call.get("destination_number", "Unknown"))
        .replace("_", "\\_")
        .replace("*", "\\*")
    )
    ...
    friendly_hangup = HANGUP_CAUSES.get(
        hangup_cause, hangup_cause.replace("_", " ").title()
    )

    gateway = (
        str(call.get("gateway", "Unknown")).replace("_", "\\_").replace("*", "\\*")
    )
            ...
            for file in files:
                if file.endswith((".wav", ".mp3")):
                    ...
                    if os.path.isfile(file_path) and os.access(file_path, os.R_OK):
                        ...
                        logger.debug(
                            f"Found audio file: {file}, size: {os.path.getsize(file_path)} bytes"
                        )
...
# Create a dictionary to track busy users
user_locks = {}
user_locks_mutex = Lock()


def prevent_concurrent_requests(func):
        ...
        with user_locks_mutex:
            if user_id in user_locks and user_locks[user_id]:
                ...
                message.bot.reply_to(
                    message, 
                    "⏳ I'm busy processing your previous request. Please wait or cancel it.",
                    reply_markup=markup
                )
                return
...
def send_progress_update(chat_id, message_id, text, bot, progress=0):
    """Update progress bar for long operations"""
